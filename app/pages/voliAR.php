...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!$err) {
        ...
        try {
            ...
            $sql = "SELECT * FROM volo
                    WHERE IATAP IN ($lista_codici_partenza_sql) 
                    AND IATAA IN ($lista_codici_arrivo_sql) 
                    AND DataP = '$dataP'
                    
                    UNION
                    
                    SELECT * FROM volo
                    WHERE IATAP IN ($lista_codici_arrivo_sql) 
                    AND IATAA IN ($lista_codici_partenza_sql) 
                    AND DataP = '$dataR'";
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                echo '  <td class="py-4 px-6">' . $row["IATAP"] . '</td>';
                                echo '  <td class="py-4 px-6">' . $row["IATAA"] . '</td>';
                                echo '  <td class="py-4 px-6">' . $row["DataP"] . '</td>';
                                echo '  <td class="py-4 px-6 font-semibold text-gray-900">€' . $row["Prezzo"] . '</td>';
                                echo '  <td class="py-4 px-6 text-center">';
                                echo '      <a href="#" class="bg-[#c88b80] hover:bg-[#b0786d] text-white px-4 py-2 rounded-lg font-semibold text-sm transition shadow-md">Prenota</a>';
                                echo '  </td>';
                                echo '</tr>';
                            }
                        } else {
